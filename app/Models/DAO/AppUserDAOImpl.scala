...
/**
  ...
  */
class AppUserDAOImpl @Inject()(protected val dbConfigProvider: DatabaseConfigProvider) extends AppUserDAO with DAOSlick {
  ...
  def find(loginInfo: LoginInfo) : Future[Option[AppUser]] = {
    val userQuery = for {
      ...
      dbUser <- slickDBUsers.filter(_.id === dbUserLoginInfo.userID)
    } yield dbUser
    db.run(userQuery.result.headOption).map { dbUserOption =>
      dbUserOption.map { user =>
        AppUser(UUID.fromString(user.userID), loginInfo, user.firstName, user.lastName, user.fullName, user.email)
      }
    }
  }
  ...
  def find(userID: UUID) : Future[Option[AppUser]] = {
    val query = for {
      dbUser <- slickDBUsers.filter(_.id === userID.toString)
  ...
  def save(user: AppUser) = {
    ...
    val actions = (for {
      _ <- slickDBUsers.insertOrUpdate(dbUser)
